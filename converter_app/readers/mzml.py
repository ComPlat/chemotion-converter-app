    ...
    def prepare_tables(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            for run in runs:
                ...
                table['header'].append('-')
                d = {k: getattr(run, k, '') for k in run.__dir__() if k[:1] != '_' and type(getattr(run, k, '')).__name__ != 'method'}
                data = ['i', 'mz']
                data_2 = ['t_mz_set','transformed_mz_with_error' ]
                ignore_fields = ['obo_translator', 'transformed_peaks']
                for key, val in d.items():
                    if key not in data + data_2 + ignore_fields:
                        if isinstance(val, ndarray):
                            ...
                            for attrib_key, attrib_val in enumerate(val):
                                table.add_metadata(key + '.' + str(attrib_key), attrib_val)
                        elif isinstance(val, ET.Element):
                            table.add_metadata(key + '.tag', val.tag)
                            table.add_metadata(key + '.text', val.text.strip())
                            for attrib_key, attrib_val in val.attrib.items():
                                table.add_metadata(key + '.' + attrib_key, attrib_val)
                        else:
                            table.add_metadata(key, str(val))

                def add_data_table(table, data_set):
                    table['columns'] = []
                    for col_idx, data_key in enumerate(data_set):
                        ...
                        table['columns'].append({'key': col_idx, 'name': data_key})

                add_data_table(table, data)
                table = self.append_table(tables)
                table['header'].append(f'ID - {d["ID"]}')
                add_data_table(table, data_2)
