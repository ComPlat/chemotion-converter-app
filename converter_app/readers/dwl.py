...
class DWLReader(Reader):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.lines = None
        self.state = DWLReader._ReaderSate()
        self.table_rows = {'__rows': 0, 'X in (Die(X/Y))': [], 'Y in (Die(X/Y))': []}

    def check(self):
        # check using separate function for inheritance
        ...
        if result:
            self._check_next_tag(self.lines[0])
        return result

    def _check_next_tag(self, row):
        match: re.Match = re.match(r'^\s*<(.+)>\s*$', row)
        if match is not None:
            ...
            if tag_name.startswith('/'):
                if tag_name[1:] == self.state.current_tags[-1]:
                    ...
                    return -1
                raise ValueError(f"Tag '{self.state.current_tags[-1]}' does not close!")
...
Readers.instance().register(DWLReader)
